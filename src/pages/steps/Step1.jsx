...
const Step1 = ({ prevStep, nextStep, formData = {}, updateFormData, setFormValidity }) => {
  useEffect(() => {
    window.scrollTo(0, 0);
  }, []);
        ...
        <div className="w-full overflow-x-auto mt-8">
          <div className="flex gap-6 min-w-max">
            {packages.map((pkg, index) => (
              <div
                key={index}
                className={`w-[270px] md:w-[300px] border border-black/10 flex flex-col rounded-2xl ${selectedPackage === pkg.name ? "border-primary" : ""}`}
                onClick={() => setSelectedPackage(pkg.name)}
              >
                <div className={`w-full h-18 ${pkg.name.includes("Package") ? "bg-primary" : "bg-secondary"} rounded-t-2xl flex items-center justify-center text-white text-center`}>
                ...
                <div className="py-6 flex flex-col gap-4 items-center justify-center">
                  <p className="text-3xl md:text-4xl font-bold">
                    {pkg.price}<span className="text-sm font-light text-black/50">NGN</span>
                  </p>
