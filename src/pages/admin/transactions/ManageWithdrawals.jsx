...
const ManageWithdrawals = () => {
  ...
  const fetchManageWithdrawalsals = async () => {
    ...
    try {
      ...
      if (response.status === 200 && response.data.success) {
        ...
        throw new Error(
          response.data.message || "Failed to fetch withdraw list."
        );
      }
    } catch (error) {
      ...
      toast.error(
        error.response?.data?.message ||
          "An error occurred fetching withdraw list!."
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleListDownloadAsCSV = async (data) => {
    ...
    try {
      ...
      const link = document.createElement("a");
      link.href = url;
      link.download = `withdrawals_for_${
        formatISODateToCustom(Date.now()).split(" ")[0]
      }.csv`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
  ...
  const handleDatePayout = async (date, data, pinCode) => {
    ...
    try {
      ...
      if (response.status === 200) {
        toast.success(
          response.data.message || `Withdrawals for ${date} paid successfully`,
          { id: toastId }
        );
        fetchManageWithdrawalsals();
      } else {
        throw new Error(
          response.data.message || `Failed to initiate withdrawal for ${date}.`
        );
      }
    } catch (error) {
      ...
      toast.error(
        error.response?.data?.message ||
          "An error occurred paying withdrawal!.",
        { id: toastId }
      );
    } finally {
      setIsPaying(false);
  ...
  return (
    <div className="space-y-6">
      {isLoading ? (
        <LazyLoader width={40} color={"green"} />
      ) : (
        Object.entries(ManageWithdrawalss).map(([date, data], index) => (
          <div key={index} className="">
            ...
            >
              ...
              <div className="flex items-center gap-4">
                ...
                <button
                  ...
                  className={`bg-primary text-tetiary h-[40px] px-4 rounded-md flex items-center gap-1 cursor-pointer ${
                    isDownloadingList ? "opacity-50 cursor-not-allowed" : ""
                  }`}
                >
            ...
            <div
              className={`overflow-x-auto transition-all duration-200 overflow-hidden styled-scrollbar ${
                activeAccordion === index ? "max-h-max" : "max-h-0"
              }`}
            >
                ...
                <tbody>
                  {data.length > 0 ? (
                    data.map((item, index) => {
                      return (
                        ...
                        >
                          ...
                          <td className="p-4 border-y border-black/10">
                            {formatterUtility(Number(item?.amount)) || "-"}
                          </td>
                          <td className="p-4 border-y border-black/10">
                            {item?.bank_name || "-"}
                          </td>
                    ...
                    <tr className="bg-white text-sm text-center">
                      <td
                        colSpan="8"
                        className="border p-4 rounded-lg border-black/10"
                      >
